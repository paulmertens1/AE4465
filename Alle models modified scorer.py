...
def asymmetric_error(y_true, y_pred, over_weight=1.0):
    error = y_pred - y_true
    weighted_error = np.where(error > 0, over_weight * error**2, error**2)
    return np.mean(weighted_error)

asymmetric_scorer = make_scorer(asymmetric_error, greater_is_better=False)


# Decision Tree
dt_params = {'max_depth': [3, 5, 10, None]}
dt_grid = GridSearchCV(DecisionTreeRegressor(), dt_params, cv=3, scoring=asymmetric_scorer)
dt_grid.fit(X_train, y_train)
best_dt = dt_grid.best_estimator_

# Random Forest
rf_params = {'n_estimators': [50, 100], 'max_depth': [5, 10, None]}
rf_grid = GridSearchCV(RandomForestRegressor(), rf_params, cv=3, scoring=asymmetric_scorer)
rf_grid.fit(X_train, y_train)
best_rf = rf_grid.best_estimator_

# XGBoost
xgb_params = {'n_estimators': [50, 100], 'max_depth': [3, 5, 10]}
xgb_grid = GridSearchCV(XGBRegressor(), xgb_params, cv=3, scoring=asymmetric_scorer)
...
# }
# grid_search = GridSearchCV(XGBRegressor(), param_grid, cv=3, scoring=asymmetric_scorer)
